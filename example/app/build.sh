...
rm app.pyz
rm installer.exe
docker run -v "$(pwd):/src/" shiv "shiv . -e app.__main__:cli -o app.pyz && chown -R --reference=. app.pyz"
...
docker run -v "$(pwd):/src/" shiv "makensis installer.nsh && chown -R --reference=. installer.exe"
